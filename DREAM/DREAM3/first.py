...
DREAM_Ns=[10,50,100]
data_set_numbers=['Ecoli1','Ecoli2','Yeast1','Yeast2','Yeast3']
...
def run_DREAM_inference(DREAM_N,data_set_number,confidence_cut=0):
    data_folder=os.path.join(analysis_folder, './DREAM3 in silico challenge/Size{0}/DREAM3 data'.format(DREAM_N))
    timeseries=pd.read_csv(os.path.join(data_folder,'InSilicoSize{0}-{1}-trajectories.tsv'.format(DREAM_N,data_set_number)),
                   sep='\t')
    
    ko=pd.read_csv(os.path.join(data_folder,'InSilicoSize{0}-{1}-null-mutants.tsv'.format(DREAM_N,data_set_number)),
                   sep='\t',index_col=0).T
